...
class NpsController {
  async execute(request: Request, response: Response) {
    ...
    const calculate = Number(
      (((promoters - detractor) / totalAnswers) * 100).toFixed(4)
    );
    ...
    const returnEstatics = await EstaticsCalculateService.execute(
      hated,
      disliked,
      indifferent,
      like,
  ...
  async calculateEstatics(request: Request, response: Response) {
    const { hated, disliked, indifferent, like, love } = request.body;

    //--------------------END NPS, start statics aplicad
    //n: number, hated: number, disliked: number, indifferent: number, like: number, love: number

    return response.json(
      await EstaticsCalculateService.execute(
        hated,
        disliked,
        indifferent,
        like,
